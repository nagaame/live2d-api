  ...
  class s {
    ...
    initializeCubism() {
      return new Promise(async (resolve, reject) => {
        try {
          // 启动 Cubism SDK
          await Live2DCubismCore.startUp();
          ...
          console.error("Failed to initialize Cubism:", err);
          reject(err);
        }
      });
    }
    ...
    async loadModel(modelId, textureId, message) {
      try {
        ...
        if (this.useCDN) {
          ...
          const modelName = this.modelList.models[modelId][textureId];
          modelPath = `${this.cdnPath}model/${modelName}/index.json`;
        } else {
          modelPath = `${this.apiPath}get/?id=${modelId}-${textureId}`;
        }

        // 使用 Cubism SDK 加载模型
        const model = await Live2DCubismFramework.Model.fromUrl(modelPath);

        // 创建渲染器
        const renderer = new Live2DCubismFramework.Renderer();
    ...
    async loadRandModel() {
      try {
        ...
        if (this.useCDN) {
          ...
          // 循环切换贴图
          textureId++;
          if (textureId >= this.modelList.models[modelId].length) {
            textureId = 0;
          }
          ...
          // 加载新贴图的模型
          if (this.modelList.models[modelId].length) {
            const modelName = this.modelList.models[modelId][textureId];
            await this.loadModel(modelId, textureId, "我的新衣服好看嘛？");
          }
        } else {
          const response = await fetch(
            `${this.apiPath}rand_textures/?id=${modelId}-${textureId}`,
          );
          const data = await response.json();

          if (data.textures.id === 1 && (textureId === 1 || textureId === 0)) {
            o("我还没有其他衣服呢！", 4000, 10);
          } else {
        ...
        console.error("Failed to load random model:", err);
      }
    }
